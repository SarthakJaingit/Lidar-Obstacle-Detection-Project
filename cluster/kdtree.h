	...
	std::vector<int>& ids, uint depth = 0)
	{
		...
		if (starting_node != NULL){
			...
			&& starting_node -> point[1] <= target[1] + distanceTol
				 ...
				 float dis = sqrt(pow(target[0] - starting_node -> point[0], 2) + pow(target[1] - starting_node -> point[1], 2)); 
				 
				 if (dis <= distanceTol){
					 ids.push_back(starting_node -> id); 
					 }
			}
			if (distanceTol + target[depth % 2] > starting_node -> point[depth % 2])
			{
				// if highest tolerance > point -> look in high tolerant area
				search_helper(target, starting_node -> right, distanceTol, ids, ++depth); 
			}
			if (target[depth % 2] - distanceTol < starting_node -> point[depth % 2])
			{
